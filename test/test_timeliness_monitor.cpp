...
void PeriodicTask(flexiv::rdk::Robot& robot, const std::vector<double>& init_pos)
{
    ...
    try {
        ...
        if (loop_counter == 5000) {
            std::cerr << "[warn] >>>>> Adding simulated loop delay <<<<<" << std::endl;
        }
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        std::cout << "Initial joint positions set to: " << flexiv::rdk::utility::Vec2Str(init_pos)
                  << std::endl;
        std::cerr << "[warn] >>>>> Simulated loop delay will be added after 5 seconds <<<<<"
                  << std::endl;
